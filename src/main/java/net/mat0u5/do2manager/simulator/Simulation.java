...
public class Simulation extends MSPTUtils {
    ...
    boolean allLevelsHat = false;


    final double emberQueueMultiplier = 1.373;
    final double treasureQueueMultiplier = 1.546;

    final double allLevelsEmberQueueMultiplier = 5.12;
    final double allLevelsTreasureQueueMultiplier = 4.603;
    ...
    public void printRunInfo() {
        ...
        if (printOut.contains("emberMAXX") || printOut.contains("all")) {
            String addToText = "\n  ";
            double emberQueueMultiplier = allLevelsHat?allLevelsEmberQueueMultiplier:this.emberQueueMultiplier;
        ...
        while (deckCopy.hasCards()) {
            ...
            if (deckCopy.permanents.size() > 0) {
                ...
                if (currentCardEffects.containsKey("cold_snap") && rnd.nextInt(4) == 0) {
                    ...
                    if (!(currentCardEffects.containsKey("cold_snap") || currentCardEffects.containsKey("bb_slippers"))) {
                ...
                if (currentCardEffects.containsKey("gambler's_paradise")) {
                    ...
                    if (rndNum == 100) {
                        ...
                        gameEffects[3] += 60;
                        treasureQ+=16;
                        emberQ+=16*((currentCardEffects.containsKey("cold_snap") || currentCardEffects.containsKey("bb_slippers"))?2:1);
                        clankBlock+=8;
                        hazardBlock+=8;
                    }
                    ...
                    if (rndNum >=82 && rndNum <=87) {
                        treasureQ+=6;
                        emberQ+=6*((currentCardEffects.containsKey("cold_snap") || currentCardEffects.containsKey("bb_slippers"))?2:1);
                    }
            ...
            if (currentCardEffects.containsKey("chill_step") && randomCard.cardName.equalsIgnoreCase("sneak")) {
                emberQ+=(2*currentCardEffects.get("chill_step")*((currentCardEffects.containsKey("cold_snap") || currentCardEffects.containsKey("bb_slippers"))?2:1));
            }
            ...
            for (String effect : effects) {
                ...
                if (effect.matches("[0-9]+E")) emberQ+=(Integer.parseInt(effect.split("E")[0])*((currentCardEffects.containsKey("cold_snap") || currentCardEffects.containsKey("bb_slippers"))?2:1));
            ...
            clankBlockActual += clankBlock;
            clankBlockActual -= clankGenerated;
            System.out.println("CB: " + clankBlockActual + "__" + randomCard.cardName);
            if (clankBlockActual < 0 && currentCardEffects.containsKey("bb_slippers")) {//TODO
                System.out.println("BB_END: " + cardsPlayed + "_" + cardsPlayedStr);
                currentCardEffects.remove("bb_slippers");
            }
            if (clankBlockActual < 0) clankBlockActual = 0;
